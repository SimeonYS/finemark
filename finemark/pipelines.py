...
class FinemarkPipeline:

    # Database setup
    conn = sqlite3.connect('finemark.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `finemark`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM finemark WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO finemark (date, title, link, content)"
